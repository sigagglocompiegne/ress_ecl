...
DROP TRIGGER t_t1_lk_point_lumineux ON m_reseau_sec.geo_v_point_lumineux;

DROP TRIGGER t_t1_point_interet ON m_reseau_sec.geo_v_pi;
...
DECLARE exploit varchar(100);
DECLARE presta varchar(100);
BEGIN


id_unique = nextval('m_reseau_sec.objet_seq');

geomzonesARC = (SELECT ST_Union(geom) AS geom FROM m_reseau_sec.geo_amt_zpne_gestion WHERE gest = 'Agglomération de la Région de Compiègne'); ---- On fait une UNION de toutes les zones de gestion ARC 

geomzonesVILLE = (SELECT ST_Union(geom) AS geom FROM m_reseau_sec.geo_amt_zpne_gestion WHERE gest = 'Commune de Compiègne');--- On fait une UNION de toutes les zones de gestion VILLE DE COMPIEGNE 
...
IF (TG_OP = 'INSERT') THEN ------ Si c'est un INSERT
	...
	IF (((SELECT count(*) FROM m_reseau_sec.geo_ecl_noeud WHERE ST_equals(NEW.geom,geom) AND situation <> '12' ) < 1 )
	   ...
	   -- S'il n'y a pas d'autre noeud dont la géométrie est égale et la situation est Actif ou inatif, ou bien s'il y a un autre noeud au même endroit et que c'est un point d'intérêt
		...
		IF (ST_Contains(geomzonesARC , NEW.geom)) THEN ------- si l'objet est dans la zone ARC, on attribue automatiquement les attributs prestataires/exploitants

			exploit = 'ARC';
			presta = 'INEO';
			

		ELSIF (ST_Contains(geomzonesVILLE , NEW.geom)) THEN--- si l'objet est dans la zone VILLE DE COMPIEGNE, on attribue automatiquement les attributs prestataires/exploitants

			exploit= 'VILLE DE COMPIEGNE';
			presta = 'LESENS';
...
ELSIF (TG_OP = 'UPDATE') THEN --------------------------------------------------- Si c'est un UPDATE
	...
	-- OU S'il y a deux noeud avec la même géométrie et que l'un est un point d'intérêt et l'autre et le noeud lui même ALORS
	-- Tous ces tests excluent les objets supprimés, mais pas les inactifs
		...
		IF (NEW.situation = '11') THEN --- Si le point lumineux devient inactif
			...
			UPDATE m_reseau_sec.geo_ecl_cable ----------------------------------- On enlève le lien avec le câble, 
			SET id_nd_ini= NULL
			WHERE ST_equals(NEW.geom,ST_StartPoint(geom)) ;
...
IF (TG_OP <> 'DELETE') THEN
	...
	IF (NEW.date_donne >= now()::timestamp) THEN ----------------------------------------------------------------- Si la date de la donnée est supérieure à la date actuelle
		...
		INSERT INTO m_reseau_sec.an_ecl_erreur (id_objet, message, heure)------------------------------------- Puis on ajoute dans la table erreur
		VALUES
		(NEW.id_pi, 'La date de création de la donnée doit être inférieure à la date actuelle', now() );-- Ce message, qui apparaît dans GEO sur la fiche
	END IF;
...
IF (TG_OP = 'INSERT') THEN 
	...
	IF (((SELECT count(*) FROM m_reseau_sec.geo_ecl_noeud WHERE ST_equals(NEW.geom,geom) AND situation <> '12') < 1 )
		...
		ELSE  ------------------------------------------------ Si l'objet n'est dans aucune zone de gestion, on met un message d'erreur
			...
			INSERT INTO m_reseau_sec.an_ecl_erreur (id_objet, message, heure)
			VALUES
			(NEW.id_pi, 'L''objet en dehors des zones de gestion. Contacter SIG pour changer les zones.', now() );
			RETURN OLD ;
		...
		NEW.insee = (SELECT insee FROM r_osm.geo_vm_osm_commune_arcba arcba WHERE ST_Contains(arcba.geom,NEW.geom));
		NEW.commune = (SELECT commune FROM r_osm.geo_vm_osm_commune_arcba arcba WHERE ST_Contains(arcba.geom,NEW.geom));
...
IF (TG_OP = 'INSERT') THEN --------------------------------------------------- Si c'est un INSERT
	...
	IF ((SELECT count(*) FROM m_reseau_sec.geo_ecl_noeud WHERE ST_equals(NEW.geom,geom) AND situation <> '12') < 1 )  THEN -- S'il n'y a pas d'autre noeud dont la géométrie est égale
		...
		IF (ST_Contains(geomzonesARC , NEW.geom)) THEN ------- si l'objet est dans la zone ARC, on attribue automatiquement les attributs prestataires/exploitants
			...
			exploit = 'ARC';
			presta = 'SEP'; --Service d'éclairage public
		...
		ELSE ------------------------------------------------ Si l'objet n'est dans aucune zone de gestion, on met un message d'erreur
			...
			INSERT INTO m_reseau_sec.an_ecl_erreur (id_objet, message, heure)
			VALUES
			(NEW.id_ouvelec, 'L''objet en dehors des zones de gestion. Contacter SIG pour change les zones.', now() );
			RETURN OLD ;
		...
		IF (NEW.ty_ouvelec <> '10' AND NEW.depart IS NOT NULL)  THEN ------------------------- Si l'ouvrage n'est pas une armoire principale et le depart n'est pas NULL	
			...
			REFRESH MATERIALIZED VIEW m_reseau_sec.an_v_materialisee_noeud_armoire; ---- On actualise la vue du "cheminement électricité"
...
ELSIF (TG_OP= 'UPDATE') THEN --------------------------------------------------- Si c'est un UPDATE
	...
	IF ((SELECT count(*) FROM m_reseau_sec.geo_ecl_noeud WHERE ST_equals(NEW.geom,geom) AND situation <> '12') < 2 )  THEN -- S'il n'y a pa d'autre noeud dont la géométrie est égale
		...
		IF (NEW.situation = '11') THEN ----------------------------------------------- Si l'objet passe en Inactif
			...
			UPDATE m_reseau_sec.geo_ecl_cable------------------------------------- On enlève les liens topologiques entre l'ouvrage et les câbles
			SET id_nd_ini= NULL
			WHERE ST_equals(NEW.geom,ST_StartPoint(geom)) ;
		...
		IF ((OLD.situation = '11' OR OLD.situation = '12') AND NEW.situation='10') THEN ---Si l'armoire est réactivée
			...
			UPDATE m_reseau_sec.geo_ecl_cable ----------------------------------- On remet les liens topolique avec les câbles.
			SET id_nd_ini= new.id_ouvelec
			WHERE ST_equals(NEW.geom,ST_StartPoint(geom)) AND situation <> '12';
